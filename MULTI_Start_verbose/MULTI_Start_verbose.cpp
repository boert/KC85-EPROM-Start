...
uint16_t calc_fsum( const std::vector<uint8_t>& data, uint16_t start, uint16_t end)
{
    uint8_t  a = 0;
    uint8_t  b = 0;
...
void convert_KCC_file( std::string kcc_filename, std::string rom_filename, std::string menuwort, int size)
{
    ...
    uint16_t block_1_fsum;
    uint16_t block_2_fsum;
    ...
    if( mem_data_compressed.size() > block_1_max_size)
    {
        ...
        block_1_fsum = calc_fsum( mem_data_compressed, 0, block_1_length);
        ...
        block_2_fsum = calc_fsum( mem_data_compressed, block_1_length, block_1_length + block_2_length);
    ...
    std::println( "Prüfsumme 1:   {:04X}h"          , block_1_fsum);
    ...
    if( block_2_length > 0)
    {
        ...
        std::println( "Prüfsumme 2:   {:04X}h"          , block_2_fsum);
    }
    else
    {
