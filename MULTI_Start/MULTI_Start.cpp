...
void help(  const std::string& self_name)
{
    ...
    std::println( "abzüglich des Hilfsprogrammes (ca. 256 Bytes).");
...
void convert_KCC_file( std::string kcc_filename, std::string rom_filename, std::string menuwort, int size)
{
    ...
    if( size == 8)
    {
        if( rom[ 0x1012] != 0x21)
        {
            std::println( "FEHLER: Hilfsprogramm modifiziert! Bitte Programmautor kontaktieren.");
            exit( EXIT_FAILURE);
        }
        // modify source address for helper program
        // on 8k rom modules
    ...
    if( mem_data.size() > block_1_max_size)
    {
        // beide Blöcke
        block_1_length = block_1_max_size;
        std::copy_n( mem_data.begin(), block_1_length, rom.begin() + block_1_start);

        block_2_length = mem_data.size() - block_1_length;
        std::copy_n( mem_data.begin() + block_1_length, block_2_length, rom.begin() + block_2_start);

    }
    else
    {
        // nur ein Block
        block_1_length = mem_data.size();
        std::copy_n( mem_data.begin(), block_1_length, rom.begin() + block_1_start);
    ...
    // Statusinformationen
    std::println( "Hilfprog 1: {:04X}h...{:04X}h", rom_offset, rom_offset + part_1_size - 1);
    std::println( "Block 1   : {:04X}h...{:04X}h", block_1_start, block_1_start + block_1_length - 1);
    std::println( "Hilfprog 2: {:04X}h...{:04X}h", rom_offset + part_2_dest, rom_offset + part_2_dest + part_2_size - 1);
    if( block_2_length > 0)
    {
