...
// mit MENU gestartet werden können
//
...
void help(  const std::string& self_name)
{
    ...
    std::println( "    M025  USER PROM 8K");
    std::println( "    M045  32k segmented ROM");
    std::println( "    M046  64k segmented ROM");
    std::println( "    M047  128k segmented ROM");
    std::println( "    M062  32k/64k seg. RAM/ROM");
    std::println( "    M125  USER PROM 8K/16K/64K");
    std::println();
    ...
    std::println( "Bei Modulen mit mehreren Segmenten kann in jedem Segment ein eigenes Programm abgelegt werden.");
    std::println();
...
void version( void)
{
    ...
    std::println( "Start mit Menüwort");
...
void convert_KCC_file( std::string kcc_filename, std::string rom_filename, std::string menuwort, int size)
{
    ...
    {
        std::println( "HINWEIS: Keine Startadresse hinterlegt, Program wird nur in den RAM geladen und nicht automatisch gestartet.");
    }
